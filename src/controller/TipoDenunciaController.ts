...
export class TipoDenunciaController {
    ...
    public atualizarTipoDenuncia = async (req: Request, res: Response) => {
        try {
            ...
            await this.tipoDenunciaBusiness.atualizarTipoDenuncia(id, nome, Number(departamento_id));

            res.status(200).send({ message: "Tipo de denúncia atualizado com sucesso!" });

        } catch (error: any) {
            console.error("Erro no Controller (atualizarTipoDenuncia):", error.message);

            if (error.message.includes("não encontrado")) {
                return res.status(404).send({ message: error.message });
            } else if (error.message.includes("obrigatórios") || error.message.includes("inválido")) {
